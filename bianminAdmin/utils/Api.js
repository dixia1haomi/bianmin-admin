...
class Api extends Base {
  ...
  // --------------------------------------- Admin ---------------------------------------
  // 登陆
  getAdminToken(data, callback) {
    this.request({ url: 'admin/getadmintoken', data: data, sCallback: (res) => { callback && callback(res) } })
  }

  // 检查adminToken是否有效
  verifyAdminToken(data, callback) {
    this.request({ url: 'admin/verifyadmintoken', data: data, sCallback: (res) => { callback && callback(res) } })
  }

// 查询formid有效的用户
  selectFormId_User(data, callback) {
    this.request({ url: 'admin/selectformiduser', data: data, sCallback: (res) => { callback && callback(res) } })
  }
